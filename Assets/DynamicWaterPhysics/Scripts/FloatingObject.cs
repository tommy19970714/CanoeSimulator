...
public partial class FloatingObject : MonoBehaviour
{
    ...
    /// </summary>
    public List<BuoyTri> underwaterTris = new List<BuoyTri>();
    ...
    void FixedUpdate()
    {
        if(water != null)
        {
            ...
            if (simulate)
            {
                ...
                if (underwaterTris.Count > 0)
                    isTouchingWater = true;
                else
                    isTouchingWater = false;

                // Apply triangle forces
                foreach (BuoyTri td in underwaterTris)
                {
                    if (td.distanceToSurface < 0)
                    {
                        rb.AddForceAtPosition(td.resultantForce, td.center);
                    }
                }
            }
    ...
    /// <returns></returns>
    private List<BuoyTri> GenerateSplitMesh()
    {
        waterLines.Clear();
        underwaterTris.Clear();
        ...
        for (int i = 0; i < dummyMeshTriCount; i += 3)
        {
            ...
            {
                underwaterTris.Add(new BuoyTri(vd0.pos, vd1.pos, vd2.pos, this, 3));
            }
    ...
    /// </summary>
    private void TwoUnderWater(VertexData[] vertexData, List<BuoyTri> underwaterTriangles)
    {
        vd0 = vertexData[0];
        vd1 = vertexData[1];
        vd2 = vertexData[2];
        ...
        // Generate tris
        t0 = new BuoyTri(M, I_M, I_L, this, 2);
        t1 = new BuoyTri(M, I_L, L, this, 2);

        // Add new waterline
        waterLines.Add(new WaterLine(I_L, I_M, t0));

        // 2 triangles below the water  
        underwaterTriangles.Add(t0);
    ...
    /// </summary>
    private void OneUnderWater(VertexData[] vertexData, List<BuoyTri> underWaterTriangleData)
    {
        vd0 = vertexData[0];
        vd1 = vertexData[1];
        vd2 = vertexData[2];
        ...
        // Generate tris
        t0 = new BuoyTri(L, J_H, J_M, this, 1);

        // Add new waterline
        waterLines.Add(new WaterLine(J_M, J_H, t0));

        underWaterTriangleData.Add(t0);
    }
    ...
    private void OnDrawGizmos()
    {
        if (DEBUG)
        {
            ...
            for (int i = 0; i < underwaterTris.Count; i++)
            {
